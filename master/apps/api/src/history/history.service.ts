...
} from 'src/common/exams/exam-content';
import {
	buildEvaluationFromAnswerMap,
	normalizeHistoryStudentAnswers,
} from 'src/common/exams/evaluation';
...
export class HistoryService {
	...
	async create(userId: string, dto: CreateHistoryDto) {
		...
		if (dto.intent === 'EXAM_PRACTICE') {
			await this.removeCompletedPracticeExam(userId, dto.exam_id);
		}
		...
		if (this.configService.get<string>('AI_API_BASE_URL')?.trim()) {
			try {
				await postToAiService(
					this.configService,
					{
						// The AI service now expects the same analysis intent for both
						// completed practice sessions and finished exam-result views.
