...
class Hoc247Spider(BaseSpider):
    ...
    def parse_listing_page(self, html: str, url: str) -> list[str]:
        return self._extractor.extract_listing_links(html, url, pattern=r"-doc\d+\.html")
    ...
    def parse_detail_page(self, html: str, url: str) -> ExamInfo:
        ...
        for sub in subjects:
            if f"mon-{sub}" in url_lower:
                info.subject = sub
                break
                
        if "lop-10" in url_lower or "-10-" in url_lower:
            info.grade = "10"
        ...
        if "giua-hk1" in url_lower:
            info.exam_type = "giua_hk1"
        elif "-hk1" in url_lower:
            info.exam_type = "hk1"
        elif "giua-hk2" in url_lower:
            info.exam_type = "giua_hk2"
        elif "-hk2" in url_lower:
            info.exam_type = "hk2"
        elif "minh-hoa" in url_lower:
            info.exam_type = "minh_hoa"
